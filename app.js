...
function renderDetail(o) {
  ...
  if (VIEWER_MODE) {
    ...
    if (o.status === "picked_up") {
      ...
      controlBox.classList.remove("hidden");
      controlBox.innerHTML = `<p style="text-align:center;font-size:13px;color:#dc2626;margin:0;">En devolución, esperando asignación</p>`;
    } else {
    ...
    controlBox.innerHTML = `
      <p style="text-align:center;font-size:13px;color:#dc2626;font-weight:600;margin:0 0 10px;">En devolución</p>
    ...
    controlBox.innerHTML = `
      ...
      <button class="secondary-btn" id="act-archive">Marcar como completada (Juan)</button>
...
async function handleReopenOrder(o) {
  ...
  if (!reason) return;
  const acting = workers.find(w => w.name === "Jorge" || w.name === "Juan");
  const session = getSession();
  const actor = workers.find(w => session && w.id === session.workerId && (w.name === "Jorge" || w.name === "Juan")) || acting;
...
async function handleAssignFromDevolucion(o) {
  ...
  if (!targetId) return;
  const session = getSession();
  const actor = workers.find(w => session && w.id === session.workerId && (w.name === "Jorge" || w.name === "Juan"))
    || workers.find(w => w.name === "Juan");
  const pin = await askPin(actor.name);
  if (!pin) return;
