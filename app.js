...
async function loadOrders() {
  ...
  const { data, error } = await sb
    ...
    .or(`status.neq.picked_up,and(status.eq.picked_up,updated_at.gte.${twoDaysAgo})`)
    .order("order_number");
...
function renderDetail(o) {
  ...
  if (activeAssignments.length === 0) {
    ...
    rowsEl.innerHTML = activeAssignments.map(a => {
      ...
      const isMine = getSession() && a.worker_id === getSession().workerId;
      let btns = "";
      if (isMine) {
...
async function handleTransferAssignment(a, o) {
  const session = getSession();
  const toId = await askWorker("¿A quién se transfiere?", w => w.id !== session.workerId);
  if (!toId) return;
...
(async function init() {
  ...
  await loadOrders();
  setInterval(() => {
    if (!getSession()) { location.reload(); return; }
